...
namespace WTF {
    ...
    template<typename T, typename U, typename V>
    inline std::pair<typename HashSet<T, U, V>::iterator, bool> HashSet<T, U, V>::add(const ValueType& value)
    {
        typedef typename HashSet<T, U, V>::iterator AddIterator;
        std::pair<typename HashTableType::iterator, bool> result = m_impl.add(value);
        return std::pair<AddIterator, bool>(AddIterator(result.first), result.second);
    }
    ...
    {
        typedef typename HashSet<Value, HashFunctions, Traits>::iterator AddIterator;
        typedef HashSetTranslatorAdapter<ValueType, ValueTraits, T, HashTranslator> Adapter;
        
        // 1. Принимаем результат функции во временную переменную
        pair<typename HashTableType::iterator, bool> result = m_impl.template addPassingHashCode<T, T, Adapter>(value, value);
        
        // 2. Явно приводим внутренний итератор хэш-таблицы к константной паре HashSet
        return pair<AddIterator, bool>(AddIterator(result.first), result.second);
    }
