/****************************************************************************
** Meta object code from reading C++ file 'changeproperties.h'
**
** Created by: The Qt Meta Object Compiler version 63 (Qt 4.8.5)
**
** WARNING! All changes made in this file will be lost!
*****************************************************************************/

#include "../../../changeproperties.h"
#if !defined(Q_MOC_OUTPUT_REVISION)
#error "The header file 'changeproperties.h' doesn't include <QObject>."
#elif Q_MOC_OUTPUT_REVISION != 63
#error "This file was generated using the moc from 4.8.5. It"
#error "cannot be used with the include files from this version of Qt."
#error "(The moc has changed too much.)"
#endif

QT_BEGIN_MOC_NAMESPACE
static const uint qt_meta_data_ChangeProperties[] = {

 // content:
       6,       // revision
       0,       // classname
       0,    0, // classinfo
       4,   14, // methods
       0,    0, // properties
       0,    0, // enums/sets
       0,    0, // constructors
       0,       // flags
       0,       // signalCount

 // slots: signature, parameters, type, tag, flags
      18,   17,   17,   17, 0x0a,
      45,   37,   17,   17, 0x09,
     105,  100,   17,   17, 0x09,
     155,   17,   17,   17, 0x09,

       0        // eod
};

static const char qt_meta_stringdata_ChangeProperties[] = {
    "ChangeProperties\0\0updateProperties()\0"
    "current\0on_listProperties_currentItemChanged(QTreeWidgetItem*)\0"
    "item\0on_listEditRequests_itemChanged(QTreeWidgetItem*)\0"
    "on_buttonSet_clicked()\0"
};

void ChangeProperties::qt_static_metacall(QObject *_o, QMetaObject::Call _c, int _id, void **_a)
{
    if (_c == QMetaObject::InvokeMetaMethod) {
        Q_ASSERT(staticMetaObject.cast(_o));
        ChangeProperties *_t = static_cast<ChangeProperties *>(_o);
        switch (_id) {
        case 0: _t->updateProperties(); break;
        case 1: _t->on_listProperties_currentItemChanged((*reinterpret_cast< QTreeWidgetItem*(*)>(_a[1]))); break;
        case 2: _t->on_listEditRequests_itemChanged((*reinterpret_cast< QTreeWidgetItem*(*)>(_a[1]))); break;
        case 3: _t->on_buttonSet_clicked(); break;
        default: ;
        }
    }
}

const QMetaObjectExtraData ChangeProperties::staticMetaObjectExtraData = {
    0,  qt_static_metacall 
};

const QMetaObject ChangeProperties::staticMetaObject = {
    { &QDialog::staticMetaObject, qt_meta_stringdata_ChangeProperties,
      qt_meta_data_ChangeProperties, &staticMetaObjectExtraData }
};

#ifdef Q_NO_DATA_RELOCATION
const QMetaObject &ChangeProperties::getStaticMetaObject() { return staticMetaObject; }
#endif //Q_NO_DATA_RELOCATION

const QMetaObject *ChangeProperties::metaObject() const
{
    return QObject::d_ptr->metaObject ? QObject::d_ptr->metaObject : &staticMetaObject;
}

void *ChangeProperties::qt_metacast(const char *_clname)
{
    if (!_clname) return 0;
    if (!strcmp(_clname, qt_meta_stringdata_ChangeProperties))
        return static_cast<void*>(const_cast< ChangeProperties*>(this));
    if (!strcmp(_clname, "Ui::ChangeProperties"))
        return static_cast< Ui::ChangeProperties*>(const_cast< ChangeProperties*>(this));
    return QDialog::qt_metacast(_clname);
}

int ChangeProperties::qt_metacall(QMetaObject::Call _c, int _id, void **_a)
{
    _id = QDialog::qt_metacall(_c, _id, _a);
    if (_id < 0)
        return _id;
    if (_c == QMetaObject::InvokeMetaMethod) {
        if (_id < 4)
            qt_static_metacall(this, _c, _id, _a);
        _id -= 4;
    }
    return _id;
}
QT_END_MOC_NAMESPACE
